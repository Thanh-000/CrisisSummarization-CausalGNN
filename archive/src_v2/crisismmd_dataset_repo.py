...
class CrisisMMDataset(BaseDataset):
    ...
    def read_data(self, ann_file):
        ...
        for l in self.info:
            ...
            #print(trigger_words)
            caption = tweet_text
            # if LLaVA_text == None or LLaVA_text == "":
    ...
    def tokenize(self, sentence):
        ...
        ids = self.tokenizer(clean_text(
            sentence), padding='max_length', max_length=77, truncation=True).items()
        return {k: torch.tensor(v) for k, v in ids}
    
    def tokenize_llamma(self, sentence):
        ids = self.llamma_tokenizer(clean_text(
            sentence), padding='max_length', max_length=128, truncation=True).items()
        return {k: torch.tensor(v) for k, v in ids}
    
    def tokenize_bert(self, sentence):
        ids = self.electra_tokenizer(clean_text(
            sentence), padding='max_length', max_length=128, truncation=True).items()
        return {k: torch.tensor(v) for k, v in ids}

    def tokenize_deepseek(self, sentence):
        ids = self.deepseek_tokenizer(clean_text(
            sentence), padding='max_length', max_length=128, truncation=True).items()
        return {k: torch.tensor(v) for k, v in ids}
    ...
    def initialize(self, opt, phase='train', cat='all', task='task2', shuffle=False, consistent_only=False):
        ...
        self.electra_tokenizer = ElectraTokenizer.from_pretrained('google/electra-base-discriminator')
        ...
        if self.deepseek_tokenizer.pad_token is None:
            self.deepseek_tokenizer.pad_token = self.deepseek_tokenizer.eos_token

        #self.tokenizer = AlbertTokenizer.from_pretrained('albert-base-v2')
        ann_file = '%s/crisismmd_datasplit_settingA/task_%s_text_img_%s.tsv' % (
    ...
    def __getitem__(self, index):
        ...
        with Image.open(data['path_image']).convert('RGB') as img:
            if self.mode == 'test':
                image = self.test_transforms(img)
            else:
