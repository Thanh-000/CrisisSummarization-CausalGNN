...
def run_geda_experiment(
    ...
    """
    from ..models.geda_model import GEDAModel, GEDALoss
    ...
    # Calculate class weights safely to avoid missing class errors in few-shot
    labeled_indices = torch.where(labeled_mask[:n_train])[0].cpu().numpy()
    labeled_subset_labels = train_labels[labeled_indices]
    classes = np.unique(all_labels)
    counts = np.bincount(labeled_subset_labels, minlength=len(classes))
    counts = np.maximum(counts, 1) # prevent div/0
