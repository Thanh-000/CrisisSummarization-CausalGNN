...
from ..models.geda_model import GEDAModel, GEDALoss, model_summary, ABLATION_CONFIGS
from ..trainers.geda_trainer import (
    run_geda_experiment,
...
def phase8_compare(args):
    ...
    for task_id in TASKS:
        ...
        if not baselines:
            print("  (No baseline models found in CSV. Did you run Phase 4 & 5?)")
            continue

        for bl in baselines:
            bl_scores = df[(df["model"] == bl) & (df["task"] == task_id)][METRIC].values
            if len(bl_scores) < 2:
                print(f"  GEDA vs {bl:<20} -- Skipped: Not enough runs (N={len(bl_scores)})")
                continue
