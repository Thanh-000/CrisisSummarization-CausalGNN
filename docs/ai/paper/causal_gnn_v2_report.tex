...
Sự phân tách $X_c$ và $X_s$ được giám sát chặt chẽ thông qua hàm mất mát tổng hợp:
\begin{equation}
\mathcal{L}_{Total} = \mathcal{L}_{Task}(X_c) + \alpha \cdot \mathcal{L}_{Spurious}(X_s)
\end{equation}
Trong đó $\mathcal{L}_{Task}$ đào tạo nhánh Nhân quả để phục vụ dự đoán nhãn chính (BCE/Cross-Entropy), trong khi $\mathcal{L}_{Spurious}$ tối ưu hóa rẽ nhánh $X_s$ bắt buộc học bối cảnh (context/domain). Mạng Adversarial (GRL) được cài cắm để ép $X_c \perp X_s$.
