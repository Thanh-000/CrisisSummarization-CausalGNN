...
\begin{figure}[H]
    ...
    \node[box, right=of proj_img, yshift=-0.9cm] (unified) {Concat / Fusion \\ $z_{unified}$};
    
    % Disentanglement
    \node[causal, right=of unified, yshift=1.2cm] (xc) {Causal Branch \\ $X_c$};
    \node[spurious, right=of unified, yshift=-1.2cm] (xs) {Spurious Branch \\ $X_s$};
    
    % Graph & Memory
    \node[graph, right=of xc] (sage) {k-NN Graph \\ GraphSAGE ($X_c^{graph}$)};
    \node[spurious, right=of xs] (bank) {Memory Bank \\ (Size: 2000)};
    
    % Causal Inference
    \node[box, right=of sage, yshift=-1.2cm, fill=orange!10, draw=orange!70] (ba) {Backdoor Adjustment \\ $\mathbb{E}_{X_s} [P(Y | do(X_c))]$};
    \node[box, right=of ba] (output) {Robust \\ Logits};
    ...
    \draw[arrow] (sage) -- (ba);
    \draw[arrow, dashed] (bank) -- (ba) node[midway, right, font=\scriptsize, text=red] {Sample N=50};
